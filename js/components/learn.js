...
export default {
    setup() {
        ...
        const keyboard = (event) => { if (event.key === 'Enter' && feedback.value) next(); };
        let timer;
    ...
    template: `
      <div class="max-w-3xl mx-auto pb-20">
        ...
        <div v-if="!completed" class="mt-4 grid grid-cols-4 gap-2 text-center text-xs font-bold text-gray-500"><span>Đúng<br><b class="text-gray-900">{{ stats.correct }}</b></span><span>Chuỗi<br><b class="text-gray-900">{{ stats.streak }}</b></span><span>Còn lại<br><b class="text-gray-900">{{ session.length - index - 1 }}</b></span><span>XP<br><b class="text-gray-900">{{ stats.xp }}</b></span></div>
        <div v-if="showSettings" class="fixed inset-0 z-50 bg-black/40 p-4 flex justify-end" @click.self="showSettings = false"><aside role="dialog" aria-modal="true" aria-label="Cài đặt Learn Mode" class="w-full max-w-md h-full overflow-y-auto bg-white rounded-3xl p-6"><div class="flex justify-between"><h2 class="text-xl font-bold">Cài đặt phiên học</h2><button @click="showSettings = false" class="icon-button" aria-label="Đóng cài đặt"><i class="fa-solid fa-xmark"></i></button></div><div class="mt-6 space-y-6"><label class="flex justify-between items-center"><span><b>Xáo trộn từ</b><small class="block text-gray-500">Trộn thứ tự thẻ</small></span><input type="checkbox" v-model="settings.shuffle" @change="saveSettings"></label><label class="flex justify-between items-center" :class="!starredCount ? 'opacity-50' : ''"><span><b>Chỉ từ đánh dấu sao</b><small class="block text-gray-500">{{ starredCount ? starredCount + ' từ có sẵn' : 'Không có từ vựng được đánh dấu sao.' }}</small></span><input type="checkbox" v-model="settings.starredOnly" @change="saveSettings" :disabled="!starredCount"></label><label class="flex justify-between items-center"><span><b>Âm thanh hiệu ứng</b><small class="block text-gray-500">Không ảnh hưởng phát âm</small></span><input type="checkbox" v-model="settings.soundEffects" @change="saveSettings"></label><div><b>Loại câu hỏi</b><div class="mt-2 space-y-2"><label v-for="(label,key) in {multiple_choice:'Trắc nghiệm',typing:'Gõ đáp án',flashcard:'Flashcard'}" :key="key" class="flex gap-2"><input type="checkbox" :checked="settings.questionTypes[key]" @change="toggleOption('questionTypes', key)">{{ label }}</label></div></div><div><b>Chiều trả lời</b><div class="mt-2 space-y-2"><label class="flex gap-2"><input type="checkbox" :checked="settings.directions.en_to_vi" @change="toggleOption('directions','en_to_vi')">Anh → Việt</label><label class="flex gap-2"><input type="checkbox" :checked="settings.directions.vi_to_en" @change="toggleOption('directions','vi_to_en')">Việt → Anh</label></div></div><button @click="startSession(); showSettings = false" class="btn-primary w-full py-3">Áp dụng và bắt đầu lại</button></div></aside></div>
      </div>`
};
